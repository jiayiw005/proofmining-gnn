...
# Load state-node semantic LM bank
...
def extract_node_features(G):
    ...
    for n in G.nodes():
        ...
        if ntype == "state":
            s = str(n)
            lm_id = STATE_LM_INDEX.get(s, -1)
        else:
            lm_id = -1

        state_lm_ids.append(lm_id)

    return (
        torch.tensor(node_type_ids, dtype=torch.long),
        torch.tensor(node_tactic_ids, dtype=torch.long),
        torch.tensor(state_lm_ids, dtype=torch.long),
    )
...
def convert_graph(fp):
    ...
    node_type_ids, node_tactic_ids, state_lm_ids = extract_node_features(G)

    data.node_type = node_type_ids
    data.node_tactic_id = node_tactic_ids
    data.state_lm_id = state_lm_ids
...
# split graph conversion
def convert_split(split):
    in_dir = Path(f"data/processed/{split}")
    out_dir = Path(f"data/pyg_semantic/{split}")
    out_dir.mkdir(parents=True, exist_ok=True)

    fps = list(in_dir.glob("*.gpickle"))
    print(f"[{split}] Converting {len(fps)} graphs with semantic state features...")
